...
ApplicationConfig::ApplicationConfig()
    : _type{UserType::ANONYMOUS}, _email{""}, _loggedIn{false}, _filename{""},
      _user{nullptr} {}
...
bool ApplicationConfig::loadSetting(const QString &filename) {
  ...
  if (!file.exists())
    writeSetting("", UserType::ANONYMOUS);
...
bool ApplicationConfig::writeSetting(const QString email, UserType type) {
  QFile file(_filename);
  if (!file.open(QFile::WriteOnly)) {
    qDebug() << file.errorString();
    return false;
  }
  QJsonObject json{{"email", email}, {"type", QString((char)type)}};
...
void ApplicationConfig::writeUser() {
  ...
  writeSetting(QString(email.c_str()), _type);
}
